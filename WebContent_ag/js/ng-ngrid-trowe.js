/*
 * This source code is purely based and inspired from ng-ngrid project on GitHub with permission from the author.
 * Original project Author: Nikhil  Mujumdar
 * Original project Url: https://github.com/nikhilogic/ngNGrid
 * Original project Demo : http://nikhilogic.github.io
 * Nuget Package Link: https://www.nuget.org/packages/ngNGrid 
 */
angular.module('ngNgridTrowe', ['ui.bootstrap', 'ngAnimate'])
.directive('ngNgridTrowe', function ($filter, $window, $timeout, $http) {
    ...
    function link(scope, element, attrs) {
        ...
        scope.pageSizeOptions = [1,5,10,15,20,25,50,100,200,500,1000];
        ...
        */
        ...
        scope.expandPlus = function(row) {
        	console.log(row.srvAgtExeId);
        	console.log(row.isNgNgridOpen);
        	if(row.isNgNgridOpen)
        		scope.onExpandPlus({row : row});
			/*$http({
	            method : 'GET',
	            url : "rest/sa/getDefDetails/" + row.srvAgtExeId
			    }).success(function(data, status, headers, config) {
			    		console.log("In success ");
			    		console.log(data);
			            row.srvAgtExeActs = data.srvAgtExeActs;
			            row.srvAgtExePrms = data.srvAgtExePrms;
    ...
    return {
        ...
        templateUrl: 'template/NgNgridTroweTemplate.html',
        link: link
    };
})
