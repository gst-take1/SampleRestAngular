URLBuilderApp.controller('ModPubTemplateController', ['$scope', '$http', '$filter', 'selectorProcessor', 'eventProcessor', function($scope, $http, $filter, selectorProcessor, eventProcessor) {
	...
	$scope.displayEvent = isEdit() ? '' : 'basic';
	$scope.basicDisabledEvent = false;
	$scope.pubTemplate = {};
	
	$scope.daFieldValues=[];
	$scope.origParams = [];
	
	$scope.help_main = 'This page lets user create publication definitions to fetch data from IDW on availability, and post it to a directory as a delimited file';
	
	$scope.initFilter = function() {
    	var data = '{"group": {"operator": "and","rules": []}}';
    	$scope.filter = JSON.parse(data);
    	$scope.filterEvent = JSON.parse(data);
    }
   ...
   $scope.$watch('filter', function (newValue) {
	...
	$scope.init = function() {
		...
		if(isEdit()) {
			...
			$scope.getPubTemplateFromKey(owner, name);
		}
	...
	$scope.savePub = function() {
		$scope.successMsg = '';
		$scope.errorMsg = '';
		$scope.formPubTemplateObj();				
		$http.post(
			  'qb/pubModify/savePubTemplate',
			  $scope.pubTemplate
		).then(function successCallback(response) {
			$scope.successMsg = "Publication Template was Saved Successfully"
		  }, function errorCallback(response) {
	...
	$scope.insertPub = function() {
		...
		$scope.formPubTemplateObj();				
		$http.post(
			  'qb/pubModify/insertPubTemplate',
			  $scope.pubTemplate
		).then(function successCallback(response) {
			$scope.successMsg = "Publication Template was Inserted Successfully"
		  }, function errorCallback(response) {
			  console.log(response);
			  $scope.errorMsg = getErrorMsgResponse(response);
		  }); 
	...
	$scope.formPubTemplateObj = function() {
		var queryTemplate = {
				entity : $scope.entity,
				selector : $scope.selector,
				resultFormat : $scope.contentType,
		...
		/*for(var i =0; i < $scope.params.length; i++) {
			var pVal = $scope.params[i].overrideArgValue ? $scope.params[i].overrideArgValue : $scope.params[i].argValue;
			queryTemplate.paramsList.push({ dataType: $scope.params[i].dataType, paramName: $scope.params[i].paramName, argValue: pVal  });
		}*/
		$scope.pubTemplate.queryTemplate = queryTemplate;
		$scope.pubTemplate.pubName = $scope.templateName;
		$scope.pubTemplate.pubOwner = $scope.owner;
		$scope.pubTemplate.event = $scope.event;
	}
	
	$scope.getPubTemplateFromKey = function(owner, name) {
		console.log("in getPubTemplateFromKey");
		$http({
			  method: 'GET',
			  url: 'qb/pubBuilder/getPubTemplateFromKey/' + owner + '/' + name
		}).then(function successCallback(response) {
			console.log(response.data);
			$scope.pubTemplate = response.data;
			//$scope.eventToFilter();
			selectorProcessor.populateQueryTemplateDetails($scope, response.data.queryTemplate);
	...
	$scope.deletePub = function() {
		...
		$scope.formPubTemplateObj();
		$http.post(
			  'qb/pubModify/deletePubTemplate',
			  $scope.pubTemplate
		).then(function successCallback(response) {
			//console.log(response.data);
			$scope.successMsg = "Publication Template was Deleted Successfully"
		  }, function errorCallback(response) {
			 $scope.errorMsg = getErrorMsgResponse(response);
		  }); 
	}
	...
	$scope.getSymbols = function () {
    	$http({
			  method: 'GET',
			  url: 'qb/pubBuilder/getSymbols'
      	}).then(function successCallback(response) {
          		//console.log(response.data);
          		$scope.symbols = response.data;
          		$scope.emptyResultList = $filter('filter')($scope.symbols, {symbolType : 'empty_result'});;
          		if(!$scope.pubTemplate.emptyResult) $scope.pubTemplate.emptyResult = $scope.emptyResultList[0].symbolName;
          		$scope.daFieldList = $filter('filter')($scope.symbols, {symbolType : 'data_available_field'});
          		for(var i =0; i<$scope.daFieldList.length; i++) {
          			$scope.getDAFieldValues($scope.daFieldList[i].symbolName);
      			}
          		
      		},function errorCallback(response) {
      			$scope.initError=getErrorMsgResponse(response);
      	});
    }
	
	$scope.getUserDefinedSymbols = function () {
    	$http({
			  method: 'GET',
			  url: 'qb/pubBuilder/getUserDefinedSymbols/' + $scope.owner
      	}).then(function successCallback(response) {
          		//console.log(response.data);
          		var symbols = response.data;
          		$scope.dlvLocList = $filter('filter')(symbols, {symbolType : 'delivery_location'});
          		if(!$scope.pubTemplate.dlvLoc) $scope.pubTemplate.dlvLoc = $scope.dlvLocList[0].symbolName;
      		},function errorCallback(response) {
      			$scope.step8Error=getErrorMsgResponse(response);
      	});
    }

	$scope.getDAFieldValues = function (symbolName) {
		$http({
			  method: 'GET',
			  url: 'qb/pubBuilder/getDAFieldValues/' + symbolName
    	}).then(function successCallback(response) {
        		//console.log(response.data);
        		$scope.daFieldValues.push({field: symbolName, values:response.data});
        		if($scope.daFieldValues.length == $scope.daFieldList.length) {
        			if(isEdit()) {
        				//populate filterEvent Obj
        				$scope.eventToFilter();
        			}
        		}
    		},function errorCallback(response) {
	...
	$scope.$watch('filterEvent', function (newValue) {
	    if(newValue){
		    try{	
		        $scope.json = JSON.stringify(newValue, null, 2);
		        //console.log($scope.json);
		        $scope.eventFormattedBasic = computedEvent(newValue.group, true);
		        $scope.event = $scope.eventFormattedBasic.replace(/<strong>/g,'').replace(/<\/strong>/g,'');
		    } catch (ex) {
				$scope.displayEvent = 'advanced';
				$scope.basicDisabledEvent = true;
				console.log('Caught exception in watch filter');
				console.error(ex);
			}
	    }
	...
	/*$scope.$watch('event', function (newValue, oldValue) {
	  	  if(newValue != undefined) {  		  	 
	  		  	$scope.eventFormattedAdv = eventProcessor.formatEventAdv(newValue);
	  	  }
		});*/
	
	$scope.eventToFilter = function () {
		try{
			if(!$scope.pubTemplate.event)
				$scope.pubTemplate.event = '';
			var origEvent = $scope.pubTemplate.event;
			var eventCp = $scope.pubTemplate.event;
			
			processGroupEvent(eventCp, $scope.filterEvent.group, $scope.connectiveOp, $scope.daFieldList, $scope.daFieldValues);
	    	//console.log($scope.filter);
	    	//console.log($scope.selector);
	    	//console.log(origSelector);
	    	if($scope.pubTemplate.event != origEvent) {
	    		console.log('Event after processGroupEvent does not match with origSelector');
	    		console.log($scope.pubTemplate.event);
	    		console.log(origEvent);
	    	}
		} catch (ex) {
			$scope.pubTemplate.event = origEvent;
			console.log('Caught exception in eventToFilter');
			console.error(ex);
		}
	}
	
	$scope.$watch('pubTemplate.dlvLoc', function (newValue, oldValue) {
		console.log('Watch pubTemplate.dlvLoc Called');
		if(newValue && oldValue && newValue != oldValue && $scope.sizeType) {		
			$scope.generateFileName();
		}
	})
	
	$scope.$watch('sizeType', function (newValue, oldValue) {
		console.log('Watch sizeType Called');
		if(newValue && newValue != oldValue) {		
			$scope.generateFileName();
		}
	})
	...
	$scope.generateFileName = function () {
		var entity = $scope.pubTemplate.dlvLoc.indexOf('/') != -1 ? $scope.pubTemplate.dlvLoc.replace(/.*\/(.*)/, '$1') : 'Custom';
		var arr1 = $scope.entity.match(/(\w*)(Publication).*/);
		var CIMEntity = arr1[1];
		var arr = $scope.entity.match(/(\d)\/(\d)/);
		var CIMVersion = arr[1] + '.' + arr[2];
		$scope.pubTemplate.fileNm = entity + '_' + CIMEntity + '_' + CIMVersion + '_YYYYMMDDTHHMISS_' + $scope.sizeType + '.dat'
	}
	
	$scope.$watch('pubTemplate.fileNm', function (newValue, oldValue) {
		console.log('Watch pubTemplate.fileNmCalled');
		if(newValue && newValue != oldValue) {
			var newFileNameWithoutExt = newValue.indexOf('.') != -1 ? newValue.substring(0, newValue.lastIndexOf('.')) : newValue;
			/*var oldFileNameWithoutExt = null;
			if($scope.oldValue) {
				var oldFileNameWithoutExt = oldValue.indexOf('.') != -1 ? oldValue.substring(0, oldValue.lastIndexOf('.')) : oldValue
			}
			if(!$scope.templateName || (oldFileNameWithoutExt && $scope.templateName == $scope.pubTemplate.dlvLoc + '/'+ oldFileNameWithoutExt) ) {				
				$scope.templateName = $scope.pubTemplate.dlvLoc + '/' + newFileNameWithoutExt
			}*/
			$scope.templateName = $scope.pubTemplate.dlvLoc + '/' + newFileNameWithoutExt
		}
	})
	...
	$scope.$watch('sizeType', function (newValue, oldValue) {
		...
		if(newValue && newValue != oldValue) {		
			...
			var arr = $scope.entity.match(/(\d)\/(\d)/);
			var CIMVersion = arr[1] + '.' + arr[2];
			$scope.pubTemplate.fileNm = entity + '_' + CIMEntity + '_' + CIMVersion + '_YYYYMMDDTHHMISS_' + newValue + '.dat'
		}
	})
